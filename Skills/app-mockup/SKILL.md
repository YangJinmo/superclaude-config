...
### 기기별 해상도 참고 표 (매번 검색하지 말고 바로 이 표를 사용)
CSS 프레임의 화면비(aspect-ratio)와 상단 컷아웃 종류를 기종명만으로 바로 정할 때 쓴다. 표에 없는 신형 기종이 나오면 WebSearch로 해상도만 확인해서 추가한다 (`{모델명} screen resolution pixels`).

**아이폰** (해상도는 물리 픽셀 W×H, 스케일 무관하게 화면비 계산에 사용)
...
**갤럭시 S 시리즈** (해상도는 기본 설정 기준, 모두 펀치홀 카메라)
...
스크린샷에 컷아웃이 없어서 새로 그려야 할 때 이 절대 크기부터 시도한다. 스크린샷이 있다면 항상 [갤럭시 전용 — 펀치홀]의 실측 공식(상태바 높이 기반 60%)을 우선하고, 여기 수치는 그 실측이 애매하거나 스크린샷 자체에 상태바가 없을 때만 근사치로 쓴다.
...
**다이나믹 아일랜드** (14 Pro부터 17 Pro Max/Air까지 전 기종 동일 크기 사용 — Apple이 세대별로 캡슐 자체 크기를 바꾼 적 없음)
...
- 모양: 완전한 캡슐(pill, border-radius = height/2), 화면 정중앙 상단, 상단 여백은 [기기별 해상도 참고 표]의 상태바 높이 기준(15 시리즈 기준 약 59pt 세이프에어리어 top).

**갤럭시 펀치홀 지름** (Samsung 공식 지원 페이지 기준, "pi"는 한국어 표기 관례상 mm 지름을 뜻함)
| 모델 | 지름 |
|---|---|
| S23 / S23+ | 3.34mm (공식) |
| S23 Ultra | 3.75mm (공식) |
...
- 스크린샷에 다이나믹 아일랜드/노치가 이미 포함되어 있는지 먼저 확인
  ...
  - 포함 안 됨 → [기기별 해상도 참고 표]에서 기종의 컷아웃 종류(노치/다이나믹 아일랜드)를 확인하고, [노치 / 다이나믹 아일랜드 / 펀치홀 실측 크기]의 크기·모양대로 새로 그려 넣는다
