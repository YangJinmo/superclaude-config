...
4. 반드시 원본 스크린샷을 직접 확인해서
   ...
   3. 좌측 상단에 iOS 화면 녹화로 인한 빨간 시간 표시(녹화 캡슐)가 있다면, OpenCV 인페인팅(`cv2.inpaint`, TELEA 알고리즘 + 경계 페더링)으로 빨간 캡슐을 제거한 뒤, 그 자리에 흰색 시간 텍스트(SF Compact 폰트)를 그려 넣어 정상 상태바로 복원한다.
